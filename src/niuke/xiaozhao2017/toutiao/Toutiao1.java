...
public class Toutiao1 {
	/**
	 * 不需要动态规划，思路很简单，先将数组从小到大依次排列，遍历数组找出相邻两个数差值大于10的个数，
	 * 将这个数和n相加然后对3取模，差几个补几个就行了，代码如下：
	 * @param args
	 */
	    ...
	    for (int i = 1; i < count; i++) {
	    	if (flag < 3) {
				if (array[i] - array[i - 1] <= 10) {
					flag++;
				}else if (flag == 1 && array[i] - array[i - 1] <= 20) {
					result++;
					flag = 3;//这样设置表示当前本次循环之后已经有了三个合适的，下一次循环会跳过一次
				}else {
					result += 3 - flag;
					flag = 1;
				}
